...
from query import query_data, query_geom_data, query_floats, all_floats, query_geom_floats
from query import get_track, get_metadata, METADATA_COLS, all_experiments
...
@app.route('/all_experiments.json')
def serve_all_experiments():
    return Response(json.dumps(all_experiments()), mimetype='application/json')

# float-level data and metadata accessors
...
def get_request_args():
    ...
    return low_pressure, high_pressure, start_date, end_date, experiment
...
def query_floats_json():
    low_pressure, high_pressure, start_date, end_date, experiment = get_request_args()
    float_ids = query_floats(low_pressure,high_pressure,start_date,end_date,experiment)
    return Response(json.dumps(float_ids),mimetype='application/json')
...
def get_geom_request_args():
    ...
    return geom, low_pressure, high_pressure, start_date, end_date, experiment

@app.route('/query_geom.csv')
def serve_query_geom_csv():
    geom, low_pressure, high_pressure, start_date, end_date, experiment = get_geom_request_args()
    def line_generator():
        for line in query_geom_data(geom, low_pressure, high_pressure, start_date, end_date, experiment):
            yield line + '\n'
    return Response(line_generator(),mimetype='text/csv')
...
def serve_geom_floats_json():
    geom, low_pressure, high_pressure, start_date, end_date, experiment = get_geom_request_args()
    float_ids = query_geom_floats(geom,low_pressure,high_pressure,start_date,end_date,experiment)
    return Response(json.dumps(float_ids),mimetype='application/json')
